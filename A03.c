...
static void recursive_decoder(int encoded_idx,int decoded_idx,int good_decoded_size)
{
  // put your code here!
  //TODO
  //MENSAGEM ORIGINAL FORMATO INTEIRO
  //MENSAGEM CODIFICADA FORMATO STRING
   _number_of_calls_++;
  int idxCodificado, idxDescodificado, tamanhoDecoded;
  idxCodificado=encoded_idx;
  idxDescodificado=decoded_idx;
  tamanhoDecoded=good_decoded_size;

  if (idxDescodificado-tamanhoDecoded >_max_extra_symbols_){ 
    _max_extra_symbols_=idxDescodificado-tamanhoDecoded;
  }
 
  int sizeSym;
  sizeSym=1;
  
  while (sizeSym < _c_->max_bits || idxCodificado+sizeSym-1 < strlen(_encoded_message_)){
    for (int i=0; i<_c_->n_symbols; i++){
      int t=0;

      for (t=0; _c_->data[i].codeword[t] && _encoded_message_[idxCodificado+t] && _c_->data[i].codeword[t] == _encoded_message_[idxCodificado+t]; t++); 

      // Só precisamos da variável t, não precisamos que aconteça nada

      if ((strlen(_c_->data[i].codeword) == sizeSym) && strlen(_c_->data[i].codeword)== t){
        _decoded_message_[idxDescodificado]=i;
        
        if (idxDescodificado-tamanhoDecoded==0 && _original_message_[idxDescodificado]==_decoded_message_[idxDescodificado]){
          if (_original_message_size_ == idxDescodificado+1){
            _number_of_solutions_=_number_of_solutions_+1;
            return;
          }
          recursive_decoder(idxCodificado+sizeSym, idxDescodificado+1, tamanhoDecoded+1);
        }
        else{
          recursive_decoder(idxCodificado+sizeSym,idxDescodificado+1,tamanhoDecoded);
        }
      }
    }
    sizeSym++;
